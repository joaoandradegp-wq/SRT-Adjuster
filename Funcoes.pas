...
Uses Unit1,ComCtrls,StdCtrls,Windows,Messages,IdHTTP,Forms,Mensagens,Wininet,SysUtils,Graphics,StrUtils,dialogs;
...
{VERIFICA SE UMA URL EXISTE - UPDATE}
function CheckURL(Url:String):Boolean;
{VERIFICA NOVA VERSÃO - UPDATE}
function NovoUpdate(Url:String):Boolean;
...
procedure VarGlobais(Executavel,Diretorio,Versao,Url,Blog,Facebook,PayPal:String);
...
begin
   case MessageBox(Application.Handle,pchar(Mensagens_Pequenas(32,IntToStr(cont_legenda_global+cont_video))),pchar(Application.Title), MB_ICONQUESTION+MB_YESNO+MB_DEFBUTTON2) of
   idYes:
        Result:=True;
   idNo :
        begin
...
Result:=False;
   ...
   MessageBox(Application.Handle, pchar(Mensagens_Pequenas(33,UpperCase(repetido))),pchar(Application.Title),MB_ICONERROR+MB_OK)
   else
   begin

       if cont_video > cont_legenda_global then
       MessageBox(Application.Handle, pchar(Mensagens_Pequenas(34,'')+
                                            Mensagens_Pequenas(36,IntToStr(cont_video))+
                                            Mensagens_Pequenas(37,IntToStr(cont_legenda_global))+
                                            Mensagens_Pequenas(38,'')),pchar(Application.Title),MB_ICONWARNING+MB_OK);

       if cont_legenda_global > cont_video then
       MessageBox(Application.Handle, pchar(Mensagens_Pequenas(35,'')+
                                            Mensagens_Pequenas(36,IntToStr(cont_video))+
                                            Mensagens_Pequenas(37,IntToStr(cont_legenda_global))+
                                            Mensagens_Pequenas(38,'')),pchar(Application.Title),MB_ICONWARNING+MB_OK);

       if (cont_video = 0) and (cont_legenda_global = 0) then
       MessageBox(Application.Handle, pchar(Mensagens_Pequenas(25,'')),pchar(Application.Title),MB_ICONERROR+MB_OK);
...
begin
 ...
 MessageBox(Application.Handle, pchar(Mensagens_Pequenas(28,'')), pchar(Application.Title), MB_ICONWARNING+MB_OK);
...
begin
 ...
 MessageBox(Application.Handle, pchar(Mensagens_Pequenas(29,'')), pchar(Application.Title), MB_ICONWARNING+MB_OK);
...
begin
 ...
 if linha_aux = 1 then
 Label_Linha.Caption:='1'+' '+copy(Mensagens_Pequenas(30,''),1,Length(Mensagens_Pequenas(30,''))-1)
 else
 Label_Linha.Caption:=IntToStr(linha_aux)+' '+Mensagens_Pequenas(30,'');

 if dialogo_aux = 1 then
 Label_Dialogo.Caption:='1'+' '+copy(Mensagens_Pequenas(31,''),1,Length(Mensagens_Pequenas(31,''))-1)
 else
 Label_Dialogo.Caption:=IntToStr(dialogo_aux)+' '+Mensagens_Pequenas(31,'');
...
VarHTTP:=TIdHTTP.Create(nil);
  ...
  //-------------------------------
   ...
   aux:=VarHTTP.Get('http://'+Url);
  //-------------------------------
  Except
  //-------------------------------------------------------------------------------
    if Length(aux) = 0 then
    begin
      case MessageBox(Application.Handle,pchar(Mensagens_Pequenas(17,''))
                                        ,pchar(Application.Title)
                                        ,MB_ICONQUESTION+MB_YESNO+MB_DEFBUTTON2) of
      idYes: Result:=True;
       idNo: Result:=False;
      end;
    end;
