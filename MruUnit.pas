...
begin
  ...
  try
    ...
    begin
      ...
      begin
        if ValueExists('SRT'+inttostr(i)) then
          s := readString('SRT'+inttostr(i)) else
          break;
        fFileList.Add(s);
      end;
      CloseKey;
...
begin
  ...
  if fileExists(LoadFrom) then
    ...
    try
      for i := 1 to fMaxFiles do
      begin
        s := readString('Legendas',inttostr(i),'');
        if s = '' then break;
        fFileList.Add(s);
      end;
    finally
    ...
    begin
      for i := 1 to fMaxFiles do
        if i > fFileList.Count then
          writeString('SRT'+inttostr(i),'') else
          writeString('SRT'+inttostr(i),fFileList[i-1]);
      CloseKey;
    end;
  finally
    free;
...
begin
  ...
  try
    for i := 1 to fMaxFiles do
      if i > fFileList.Count then
        writeString('Legendas',inttostr(i),'') else
        writeString('Legendas',inttostr(i),fFileList[i-1]);
  finally
    free;
  end;
end;
